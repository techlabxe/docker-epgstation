...
const output_name = path.basename(output, path.extname(output)) + '_h265';
const output_dir = path.dirname(output);

//FFmpegオプション生成 ここから
const args = ['-y'];
const preset = 'medium';
const codec = 'libx265'; //libx265でエンコード
const crf = 22;
const videoFilter = 'yadif,hqdn3d=4.0,scale=1280:720:flags=lanczos+accurate_rnd,unsharp=3:3:0.5:3:3:0.5:0,pp=ac';
...
if (isDualMono) {
    ...
    Array.prototype.push.apply(args, ['-c:a aac', '-ar 48000', '-ab 128k']);
}
...
(async () => {
    ...
    const commands = ['-n', '15', 'jlse', ...jlse_args];
    const child = spawn('/usr/bin/nice', commands, {env:env, detached: true});
